...
namespace vcl{
    ...
    typedef WideString TString;
    #else
    typedef UnicodeString TString;
    #endif
#else
...
{
    for(int i=0; i<obj->ControlCount; ++i){
        function(obj->Controls[i]);
        if(process_child){
            TWinControl* winobj = dynamic_cast<TWinControl*>(obj->Controls[i]);
            if(winobj){
...
void for_each(TWinControl* obj, vcl_stdproc function, bool process_child = false)
{
    for(int i=0; i<obj->ControlCount; ++i){
        function(obj->Controls[i]);
        if(process_child){
            TWinControl* subobj = dynamic_cast<TWinControl*>(obj->Controls[i]);
            if(subobj){
                for_each(subobj, function, process_child);
            }
        }
    }
}
