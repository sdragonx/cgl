...
void md5::update(const void* buffer, uint32_t size)
{
...
void md5::finish(void* buffer)
{
    ...
    if(buffer){
        memcpy(buffer, m_context.data, length);
    }
    tohex(m_context.str, m_context.data, length);
}

void md5::encode(uint8_t *lpOutput, const uint32_t *lpInput, uint32_t dwLength)
{
    uint32_t j, tmp;
...
void md5::decode(uint32_t *lpOutput, const uint8_t *lpInput, uint32_t dwLength)
{
    uint32_t j;
    for(j=0; j<dwLength; j+=4)
    {
...
inline uint32_t md5::FF(uint32_t A, uint32_t B, uint32_t C, uint32_t D, uint32_t X, uint32_t L, uint32_t AC)
{
    A += F(B, C, D) + X + AC;
    A  = ROTATE_LEFT(A, L);
    return A + B;
...
inline uint32_t md5::GG(uint32_t A, uint32_t B, uint32_t C, uint32_t D, uint32_t X, uint32_t L, uint32_t AC)
{
    A += G(B, C, D) + X + AC;
    A =  ROTATE_LEFT(A, L);
    return A + B;
...
inline uint32_t md5::HH(uint32_t A, uint32_t B, uint32_t C, uint32_t D, uint32_t X, uint32_t L, uint32_t AC)
{
    A += H(B, C, D) + X + AC;
    A =  ROTATE_LEFT(A, L);
    return A + B;
...
inline uint32_t md5::II(uint32_t A, uint32_t B, uint32_t C, uint32_t D, uint32_t X, uint32_t L, uint32_t AC)
{
    A += I(B, C, D) + X + AC;
    A =  ROTATE_LEFT(A, L);
    return A + B;
...
void md5::transform(const uint8_t *lpInput)
{
