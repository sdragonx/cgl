/*
 ...
 // URL: http://www.cnblogs.com/zyl910/archive/2012/08/02/printmacro.html

*/
#ifndef PREDEFINE_HPP_20200202170204
#define PREDEFINE_HPP_20200202170204
...
void print_macro(const MACRO_T* macros, int size)
{
    using namespace std;
    char buf[1024];
    for( int i = 0; i < size; ++i ){
        _snprintf(buf, 1024, "%-32s  %s", macros[i].name, macros[i].value);
        print(buf);
    }
    print("");
}
...
void compiler_test()
{
    print_macro(g_compilers, sizeof(g_compilers)/sizeof(g_compilers[0]));
    print_macro(g_platforms, sizeof(g_platforms)/sizeof(g_platforms[0]));
    print_macro(g_stdc, sizeof(g_stdc)/sizeof(g_stdc[0]));
    print_macro(g_vc, sizeof(g_vc)/sizeof(g_vc[0]));
    print_macro(g_borland, sizeof(g_borland)/sizeof(g_borland[0]));
    print_macro(g_intel_win, sizeof(g_intel_win)/sizeof(g_intel_win[0]));
    print_macro(g_intel_linux, sizeof(g_intel_linux)/sizeof(g_intel_linux[0]));
    print_macro(g_gcc, sizeof(g_gcc)/sizeof(g_gcc[0]));
    print_macro(g_others, sizeof(g_others)/sizeof(g_others[0]));
}

void fprint_macro(std::FILE* f, const MACRO_T* macros, int size)
{
    for( int i = 0; i < size; ++i ){
...
void compiler_test(const char* filename)
{
    std::FILE* f = std::fopen(filename, "a");

    std::fprintf(f, "//---------------------------------------------------------------------------\r\n");
    std::fprintf(f, "\r\n");

    fprint_macro(f, g_compilers, sizeof(g_compilers)/sizeof(g_compilers[0]));
    fprint_macro(f, g_platforms, sizeof(g_platforms)/sizeof(g_platforms[0]));
    fprint_macro(f, g_stdc, sizeof(g_stdc)/sizeof(g_stdc[0]));
    fprint_macro(f, g_vc, sizeof(g_vc)/sizeof(g_vc[0]));
    fprint_macro(f, g_borland, sizeof(g_borland)/sizeof(g_borland[0]));
    fprint_macro(f, g_intel_win, sizeof(g_intel_win)/sizeof(g_intel_win[0]));
    fprint_macro(f, g_intel_linux, sizeof(g_intel_linux)/sizeof(g_intel_linux[0]));
    fprint_macro(f, g_gcc, sizeof(g_gcc)/sizeof(g_gcc[0]));
    fprint_macro(f, g_others, sizeof(g_others)/sizeof(g_others[0]));

    std::fclose(f);
}

#endif //PREDEFINE_HPP_20200202170204