...
public:
    ...
    pointer allocate()
    {
        ++m_size;
        int page = m_size >> 10;
        if(page < poolbuf.size()){
            return &poolbuf[page][m_size & PAGE_MASK];
        }
        else{
            allocate_buffer();
            return &poolbuf[page][m_size & PAGE_MASK];
...
//object传入一个自定义对象
typedef int (*astar_callback)( int x, int y, void* object );
...
private:
    ...
    {
        m_map[pos.y * m_width + pos.x] = n;
    }

    bool is_invalid_local( const point & pos )
    {
        //return pos.x < m_org.x || pos.y < m_org.y || pos.x >= m_org.x + MAP_SIZE || pos.y >= m_org.y + MAP_SIZE;
        return !m_range.contains( pos );
    }

    bool is_invalid_world_position( const point & pos )
    {
    ...
    {
        if(m_range.width){//区域搜索
            return !is_invalid_world_position( pos ) && !is_invalid_local( pos );
        }
        else{
            return !is_invalid_world_position( pos );
        }
    }
    ...
    {
        int n = on_block_test( x, y, object );
    ...
    bool corner )    //是否斜走
{
    ...
    if( on_block_test( target_pos.x, target_pos.y, object ) < 0 ){//目标不可通过
        return false;
    }

    //斜角行走判断。如果斜角方向两边都有砖块，则不通过2019/5/26 23:26:12
    if(corner){
    ...
    int max_step)        //搜索最大步数
{
    ...
    if( !find_nearest ){
        ...
        if( on_block_test(end.x, end.y, object ) < 0 ){
            return -3;
        }
    }
    ...
    //范围测试
    if( range ){
        m_range = vec4<T>(start.x - range, start.y - range, range + range + 1, range + range + 1);
        m_range = m_range & vec4<T>(0, 0, width, height);
    }
    ...
    while ( !m_openlist.empty() ){
        ...
        if ( current_node->pos == end ){
            while ( current_node->parent ){//压入的起点，会使角色走回头路
